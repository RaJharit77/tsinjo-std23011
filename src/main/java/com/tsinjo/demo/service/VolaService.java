              ...
              try {
                ResponseEntity<VolaResponse> response =
                    restTemplate.exchange(
                        VOLA_API_URL + payment.getId(), HttpMethod.GET, entity, VolaResponse.class);

                if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                  payment.setStatus(response.getBody().getStatus());
                  paymentRepository.save(payment);
                  log.info(
                      "Payment {} updated to {}", payment.getId(), response.getBody().getStatus());
                } else {
                  log.warn(
                      "Failed to verify payment {}: {}", payment.getId(), response.getStatusCode());
                }
              } catch (Exception e) {
